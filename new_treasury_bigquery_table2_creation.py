...
def create_bigquery_table():
    ...
    # Create tables
    create_table_if_not_exists(bq_client, "treasury_PDFs", schema_tickets)
    create_table_if_not_exists(bq_client, "treasury_Attachments", schema_attachments)  # Fix: use bq_client
    # Populate tickets
    tickets_data = []
    ...
    job = bq_client.load_table_from_dataframe(
        df_tickets, bq_client.dataset(dataset_id).table("treasury_PDFs"), 
    ...
    job = bq_client.load_table_from_dataframe(
        df_attachments, bq_client.dataset(dataset_id).table("treasury_Attachments"), 
        job_config=bigquery.LoadJobConfig(write_disposition="WRITE_TRUNCATE")
    )
    job.result()
